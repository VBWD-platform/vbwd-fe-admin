/**
 * S85.4 (fe-admin) — netto/brutto price-display resolution (pure logic).
 *
 * Mirrors the fe-user `resolveDisplaySide` precedence verbatim (D9) so the
 * netto/brutto rule cannot drift between the two apps. Admin has no business
 * viewer overlay in practice, but the rule SHAPE is identical: business viewer
 * => netto; else per-item mode; else global; default brutto.
 */
